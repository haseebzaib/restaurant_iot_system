...
//lv_disp_draw_buf_t  g;
static lv_disp_drv_t disp_drv;
static lv_color_t disp_buf1[DISP_HOR_RES * DISP_VER_RES /2];
...
void Display_init_lvgl(int rotation,uint_least8_t spi_index){
    static lv_disp_draw_buf_t buf;

    lv_disp_draw_buf_init(&buf, disp_buf1, NULL, DISP_HOR_RES * DISP_VER_RES /2);
    lv_disp_drv_init(&disp_drv);

    ST7789_Init(spi_index);
...
void ST7789_Send_Data_DMA(uint16_t x, uint16_t y, uint16_t x_end, uint16_t y_end, uint8_t *p){
   ...
   // ready = 1;
    ST7789_WriteData(p,(x_end - x + 1) * (y_end - y + 1) * 2);
   // ready = 0;
}
...
#ifdef USE_CS
    GPIO_setConfig(CONFIG_GPIO_CS,GPIO_CFG_OUTPUT | GPIO_CFG_OUT_HIGH);
#endif
...
//    uint8_t data[] = {color >> 8, color & 0xFF};
//    ST7789_WriteData(data, (xEnd - xSta + 1) * (yEnd - ySta + 1) * 2);
}
...
void ST7789_Test(void)
{
    ...
    HAL_Delay(1000);
    ST7789_WriteString(35, 105, "Speed Test", Font_16x26, RED, WHITE);
    HAL_Delay(1000);
    ...
    ST7789_Fill_Color(RED);
    ST7789_WriteString(60, 60, "Rect./Line.", Font_11x18, YELLOW, BLACK);
    ST7789_DrawRectangle(80, 80, 100, 100, WHITE);
    HAL_Delay(1000);

    ST7789_Fill_Color(RED);
    ST7789_WriteString(60, 60, "Filled Rect.", Font_11x18, YELLOW, BLACK);
    ST7789_DrawFilledRectangle(80, 80, 50, 50, WHITE);
    HAL_Delay(1000);
    ...
    ST7789_Fill_Color(WHITE);
    ST7789_DrawImage(55, 75, 128, 128, (uint16_t *)saber);
    HAL_Delay(3000);
}

void ST7789_Test2(void)
{
    ST7789_Fill_Color(WHITE);
    HAL_Delay(5000);
    ST7789_WriteString(65, 40, "STYRON", Font_16x26, RED, WHITE);
    HAL_Delay(5000);
    ST7789_WriteString(35, 100, "Welcome to", Font_16x26, BLACK, WHITE);
    ST7789_WriteString(35, 150, "Speed Test", Font_16x26, BLACK, WHITE);
    HAL_Delay(5000);
    ST7789_Fill_Color(CYAN);
    HAL_Delay(5000);
    ST7789_Fill_Color(RED);
